 ...
 * <li> Identify read vs. unread posts (isRead flag filtering). </li>
 * </ul>
...
public class PostSearchTests {
    ...
    public static void testSearch_exactTitleMatch() {
        ...
        List<post> results = searchPosts(posts, "Help with HW2");
    ...
    public static void testSearch_partialTitleMatch() {
        ...
        List<post> posts = buildSamplePosts();
        List<post> results = searchPosts(posts, "HW2");
    ...
    public static void testSearch_contentKeyword() {
        ...
        List<post> posts = buildSamplePosts();
        List<post> results = searchPosts(posts, "arraylist");
    ...
    public static void testSearch_caseInsensitive() {
        ...
        check("all three return same count",upper.size() == lower.size()
                                         && lower.size() == mixed.size());
    }
    ...
    public static void testSearch_emptyKeyword_returnsAll() {
        ...
        List<post> posts = buildSamplePosts();
        // Remove deleted posts first, they shouldn't appear in search listings
        List<post> active = filterActive(posts);
        List<post> results = searchPosts(active, "");
        check("empty keyword returns all active posts", results.size() == active.size());
    }

    /**
     * Verifies that soft-deleted posts are completely excluded from the active set and search results.
     * <p>
     * <b>Intent:</b> Maintains database security and viewer privacy by ensuring that deleted content 
     * cannot be accidentally discovered or leaked through the search feature.
     * </p>
     */
    public static void testSearch_deletedPostsExcluded() {
        section("7) Search – deleted posts are excluded from search results\n");
        List<post> posts = buildSamplePosts();
        // 'deleted' post has DELETED_MARKER as title so searching for original title should not find it
        List<post> active = filterActive(posts);
    ...
    public static void testSearch_multipleMatches() {
        ...
        posts.add(new post(1, "Alice", "Question about loops", "Loops in Java?",       "t1", false));
        posts.add(new post(2, "Bob",   "How do loops work",    "I need help with them.","t2", false));
        posts.add(new post(3, "Carol", "Recursion question",   "Not related to this.", "t3", false));
    ...
    public static void testSearch_keywordInBothTitleAndContent() {
        ...
        List<post> posts = new ArrayList<>();
        posts.add(new post(1, "Alice", "Java recursion", "Recursion in Java is tricky.", "t1", false));
     ...
     * <p>
     * <b>Intent:</b> Ensures that technical queries (such as programming languages like "C++") 
     * work flawlessly without breaking the search algorithm.
     * </p>
     */
    public static void testSearch_specialCharacterKeyword() {
        section("10) Search – keyword with special chars searched correctly\n");
        List<post> posts = new ArrayList<>();
        posts.add(new post(1, "Dave", "C++ vs Java", "C++ syntax.", "t1", false));
     ...
     * <p>
     * <b>Intent:</b> Directly satisfies the explicit user story requirement that states: 
    ...
    public static void testSearch_acrossMultipleThreads() {
        ...
        thread1.add(new post(10, "Alice", "General Q",   "General body.",   "t10", false));
        List<post> thread2 = new ArrayList<>();
        thread2.add(new post(11, "Bob",   "Exam Q",      "Exam body.",      "t11", false));
        List<post> thread3 = new ArrayList<>();
        thread3.add(new post(12, "Carol", "Homework HW2","Homework body.",  "t12", false));
     ...
     * <p>
     * <b>Intent:</b> Proves the filtering logic can successfully isolate threads, allowing users to 
    ...
    public static void testSearch_singleThreadFilter() {
        ...
        List<post> thread1 = new ArrayList<>();
        thread1.add(new post(20, "Alice", "Thread1 post A", "Shared keyword.", "t20", false));
        List<post> thread2 = new ArrayList<>();
        thread2.add(new post(21, "Bob",   "Thread2 post B", "Shared keyword.", "t21", false));
     ...
     * @param keyword The lower-case keyword to search for.
    ...
    public static List<post> buildSamplePosts() {
        List<post> posts = new ArrayList<>();
        posts.add(new post(1,  "Alice", "Help with HW2",             "I need help on HW2 homework.", "t1",  false));
        posts.add(new post(2,  "Bob",   "ArrayList question",        "How do I use an arraylist?",   "t2",  false));
        posts.add(new post(3,  "Carol", "Recursion homework help",   "Stuck on recursion.",          "t3",  false));
        posts.add(new post(4,  "Dave",  "Exam study tips",           "Any tips for the final?",      "t4",  false));
        posts.add(new post(5,  "Eve",   "Java syntax question",      "When to use semicolons?",      "t5",  false));
        // A deleted post should not appear in active searches
        post deleted = new post(6, "Frank", "Old Topic", "Old body.", "t6", false);
        deleted.setDeleted(true);
        deleted.setTitle("(this post has been deleted)");
        deleted.setContent("(this post has been deleted)");
        posts.add(deleted);
        return posts;
    }
