 ...
 * <ul>
 * <li> Create a post with title and content (default thread = "General") </li>
 * <li> Update a post </li>
 * <li> Delete a post with soft-delete and reply preservation </li>
 * <li> View a list of posts </li>
 * <li> View my posts (filter by author) </li>
 * </ul>
...
public class PostOperationsTests {
     ...
     * <p>
     * <b>Intent:</b> Directly satisfies the user story "Create a post with title and content" by proving 
     * the system properly captures and stores user input upon initialization.
     * </p>
     */
    public static void testCreatePost_validInputs() {
        section("1) Create Post – valid inputs stored correctly\n");
        post p = new post(1, "Alice", "Help with HW2", "Content text here.", "2025-01-01 10:00", false);
     ...
     * <p>
     * <b>Intent:</b> Ensures that system-generated edge cases during post creation still respect the 
     * secure deletion state, preventing leaked information.
     * </p>
     */
    public static void testCreatePost_deletedFlagSetsMarkers() {
        section("2) Create Post – pre-deleted post sets deletion markers\n");
        post p = new post(2, "Bob", "Some Title", "Some content.", "2025-01-02 11:00", true);

        check("isDeleted true",            p.getDeleted());
        check("title is deletion marker",  "(this post has been deleted)".equals(p.getTitle()));
        check("content is deletion marker","(this post has been deleted)".equals(p.getContent()));
    ...
    /**
     * Verifies that all accessor (getter) methods return the exact data stored in the post object.
     * <p>
     * <b>Intent:</b> Supports the "View a list of posts" and "View my posts" user stories by guaranteeing 
     * that the backend reliably retrieves stored data for the front-end to display.
     * </p>
     */
    public static void testReadPost_getters() {
        section("3) Read Post – getters return correct values\n");
        post p = new post(5, "Carol", "My Question", "Detailed content.", "2025-03-01 08:30", false);
    ...
    /**
     * Verifies that the title and content of a post can be updated independently of other metadata.
     * <p>
     * <b>Intent:</b> Directly satisfies the "Update a post" user story by proving that mutation methods 
     * work successfully without corrupting the object's surrounding data.
     * </p>
     */
    public static void testUpdatePost_titleAndContent() {
        section("4) Update Post – title and content updated independently\n");
        post p = new post(10, "Dave", "Old Title", "Old content.", "2025-01-10 09:00", false);
    ...
    /**
     * Verifies that updating a post's title multiple times leaves the original author field perfectly intact.
     * <p>
     * <b>Intent:</b> Ensures data integrity during the "Update a post" user story, preventing 
     * edits from accidentally reassigning ownership of the post.
     * </p>
     */
    public static void testUpdatePost_authorUnchangedAfterTitleUpdate() {
        ...
        post p = new post(12, "Eve", "Draft Title", "Some content.", "2025-01-12 11:00", false);
    ...
    /**
     * Verifies that soft-deleting a post successfully sets the isDeleted flag and applies deletion markers.
     * <p>
     * <b>Intent:</b> Proves the core mechanics of the "Delete a post with soft-delete" user story operate 
     * correctly, effectively masking the post from public view.
     * </p>
     */
    public static void testDeletePost_softDelete() {
        section("6) Delete Post – soft delete sets isDeleted flag\n");
        post p = new post(20, "Frank", "Question", "Need help.", "2025-01-20 07:00", false);
        p.setDeleted(true);
        p.setTitle("(this post has been deleted)");
        p.setContent("(this post has been deleted)");

        check("isDeleted true",             p.getDeleted());
        check("title shows deletion msg",   "(this post has been deleted)".equals(p.getTitle()));
        check("content shows deletion msg", "(this post has been deleted)".equals(p.getContent()));
        check("author preserved",           "Frank".equals(p.getAuthor()));
    }

    /**
     * Verifies that a single reply attached to a post remains intact and accessible after the post is soft-deleted.
     * <p>
     * <b>Intent:</b> Satisfies the "reply preservation" condition of the delete user story, ensuring 
     * that community discussions are not accidentally erased.
     * </p>
     */
    public static void testDeletePost_repliesPreservedAfterSoftDelete() {
        ...
        post p = new post(21, "Grace", "Post With Reply", "Content.", "2025-01-21 06:00", false);
     ...
     * <p>
     * <b>Intent:</b> Stress-tests the reply preservation requirement, guaranteeing no data loss occurs 
     * when soft-deleting highly active threads.
     * </p>
     */
    public static void testDeletePost_multipleRepliesPreserved() {
        ...
        post p = new post(22, "ivan", "Multi-reply Post", "Content.", "2025-01-22 05:00", false);
    ...
    /**
     * Verifies that a post can successfully and safely transition through its entire lifecycle 
     * without crashing or corrupting.
     * <p>
     * <b>Intent:</b> Proves the overall stability and interoperability of the Create, Update, and 
     * Delete user stories when chained together in a realistic user flow.
     * </p>
     */
    public static void testStateTransition_createdToUpdatedToDeleted() {
        ...
        // Create
        post p = new post(30, "Mike", "Initial Title", "Initial content.", "2025-02-01 10:00", false);
    ...
    /**
     * Verifies that a collection of posts can be accurately filtered by a specific author's name.
     * <p>
     * <b>Intent:</b> Directly satisfies the "View my posts (filter by author)" user story by proving 
     * the system can isolate and return only the records belonging to the active user.
     * </p>
     */
    public static void testPostList_filterByAuthor() {
        ...
        allPosts.add(new post(40, "Nancy", "Nancy Q1", "Content.", "2025-03-01 10:00", false));
        allPosts.add(new post(41, "Oscar", "Oscar Q1", "Content.", "2025-03-01 11:00", false));
        allPosts.add(new post(42, "Nancy", "Nancy Q2", "Content.", "2025-03-01 12:00", false));
        allPosts.add(new post(43, "Oscar", "Oscar Q2", "Content.", "2025-03-01 13:00", false));
