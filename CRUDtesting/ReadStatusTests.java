...
public class ReadStatusTests {
     ...
     * <p>
     * <b>Intent:</b> Establishes the baseline accuracy for the user story, ensuring new users 
     * correctly see a fully unread list of posts upon their first visit.
     * </p>
     */
     ...
     * <p>
     * <b>Intent:</b> Ensures users can accurately track which specific posts they have viewed, 
     * satisfying the "see which are read" requirement without affecting unread threads.
     * </p>
     */
    ...
    /**
     * Verifies that marking the same post as read multiple times is idempotent and prevents double-counting.
     * <p>
     * <b>Intent:</b> Ensures UI stability and accurate read metrics even if a user repeatedly clicks 
     * on or visits the same post.
     * </p>
     */
    ...
    /**
     * Verifies that filtering for unread posts accurately returns only posts not yet marked as read.
     * <p>
     * <b>Intent:</b> Directly satisfies the user story requirement allowing a user to isolate and 
     * view only the posts they have not engaged with.
     * </p>
     */
    public static void testUnreadPosts_filterReturnsOnlyUnread() {
        ...
        check("unread list contains no read posts",
              unread.stream().noneMatch(p -> tracker.isPostRead(p.getPostID())));
    }

    /**
     * Verifies that filtering for read posts returns exactly the posts that were explicitly marked as read.
     * <p>
     * <b>Intent:</b> Supports the "see which are read" requirement, ensuring users can accurately 
     * review their own reading history.
     * </p>
     */
    ...
    /**
     * Verifies that a post's reply list size accurately reflects the correct reply count after new additions.
     * <p>
     * <b>Intent:</b> Directly satisfies the requirement to "see reply count" on the main post list view.
     * </p>
     */
    public static void testReplyCount_correctOnPost() {
        section("6) Reply Count – post shows correct reply count\n");
        post p = new post(10, "alice", "Q", "Body.", "t1", false);
     ...
     * <p>
     * <b>Intent:</b> Establishes the baseline unread reply count, proving the system accurately tracks 
     * fresh community input for the user.
     * </p>
     */
    public static void testUnreadReplies_allUnreadInitially() {
        section("7) Reply Read Status – all replies unread initially\n");
        post p = new post(20, "dave", "Post", "Body.", "t4", false);
    ...
    /**
     * Verifies that marking a single reply as read updates only that specific reply's read status.
     * <p>
     * <b>Intent:</b> Ensures precise, individual tracking for the "View replies and see how many are unread" 
     * user story, avoiding blanket updates that would misrepresent the user's progress.
     * </p>
     */
    ...
    /**
     * Verifies that filtering for unread replies accurately returns only the replies not yet marked as read.
     * <p>
     * <b>Intent:</b> Directly satisfies the explicit user story requirement to "filter to unread only" 
     * when viewing a busy reply thread.
     * </p>
     */
    public static void testUnreadReplies_filterReturnsOnlyUnread() {
        section("9) Reply Read Status – filter to unread replies only\n");
        post p = new post(30, "grace", "Post", "Body.", "t7", false);
    ...
    /**
     * Verifies that the total unread reply count accurately decreases as individual replies are marked read over time.
     * <p>
     * <b>Intent:</b> Satisfies the requirement to correctly "see how many are unread" as the user 
     * sequentially reads through an active discussion thread.
     * </p>
     */
    public static void testUnreadCount_decrementAfterRead() {
        ...
        post p = new post(40, "kevin", "Post", "Body.", "t11", false);
        for (int i = 1; i <= 5; i++) {
    ...
    /**
     * Verifies that read/unread status is securely tracked per individual user via independent trackers.
     * <p>
     * <b>Intent:</b> Ensures realistic multi-user environment behavior, proving that one user's 
     * reading activity does not incorrectly clear the unread flags for a different user.
     * </p>
     */
     ...
     * Demonstrates how read status could be stored independently of a database.
     */
    public static class ReadTracker {
        ...
        public void markPostRead(int postId)    { readPostIds.add(postId); }
        
        /** Checks if a post is currently marked as read.
         * @param postId The ID of the post.
         * @return True if read, false otherwise. */
        public boolean isPostRead(int postId)   { return readPostIds.contains(postId); }
        
        /** Gets the total number of posts read.
		* @return The total number of posts read by this tracker. */
        public int totalPostsRead()             { return readPostIds.size(); }

        /** Marks a reply as read by adding its ID to the read set.
         * @param replyId The ID of the reply. */
        public void markReplyRead(int replyId)  { readReplyIds.add(replyId); }
        
        /** Checks if a reply is currently marked as read.
         * @param replyId The ID of the reply.
         * @return True if read, false otherwise. */
        public boolean isReplyRead(int replyId) { return readReplyIds.contains(replyId); }
    }
    ...
    public static List<post> filterUnreadPosts(List<post> posts, ReadTracker tracker) {
        ...
        for (post p : posts) {
            if (!tracker.isPostRead(p.getPostID())) result.add(p);
        }
        return result;
    }
    ...
    public static List<reply> filterUnreadReplies(List<reply> replies, ReadTracker tracker) {
        ...
        for (reply r : replies) {
            if (!tracker.isReplyRead(r.getReplyID())) result.add(r);
        }
        return result;
    }
    ...
    public static long countUnreadReplies(List<reply> replies, ReadTracker tracker) {
        return replies.stream().filter(r -> !tracker.isReplyRead(r.getReplyID())).count();
    }
    ...
    public static List<post> buildSamplePosts() {
        ...
        posts.add(new post(1, "Alice", "Post A", "Body A.", "t1", false));
        posts.add(new post(2, "Bob",   "Post B", "Body B.", "t2", false));
        posts.add(new post(3, "Carol", "Post C", "Body C.", "t3", false));
        posts.add(new post(4, "Dave",  "Post D", "Body D.", "t4", false));
        return posts;
    }
