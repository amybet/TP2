...
public class ViewRole2Home {
	...
	Button button_createThread = new Button("Create Thread");
	Button button_editThread = new Button("Edit Thread"); 
	...
	protected static final int theRole = 2;		// Admin: 1; Role2: 2; Role2: 3
	...
	public static void displayRole2Home(Stage ps, User user) {
		...
		theStage.setScene(theViewRole2HomeScene);
		theStage.show();
	}
	...
	private ViewRole2Home() {
		...
		theViewRole2HomeScene = new Scene(theRootPane, width, height);	// Create the scene
		theViewRole2HomeScene.getStylesheets().add(getClass().getResource("/applicationMain/application.css").toExternalForm());
		...
		Button button_ViewFlags = new Button("View Flags");

		setupButtonUI(button_ViewFlags, "Dialog", 16, 150, Pos.CENTER, width - 180, 10);
		
		button_ViewFlags.setOnAction(e -> showFlagDashboard());		
		
		// GUI Area 3
        setupButtonUI(button_Logout, "Dialog", 18, 100, Pos.CENTER, 20, 540);
        button_Logout.setOnAction((_) -> {ControllerRole2Home.performLogout(); });
        
        setupButtonUI(button_Quit, "Dialog", 18, 100, Pos.CENTER, 20, 10);
        button_Quit.setOnAction((_) -> {ControllerRole2Home.performQuit(); });
     	...
     	setupButtonUI(button_CreatePost, "Dialog", 18, 150, Pos.CENTER, width-150-20, 540); // width-150-20
     	...
     	// threads
     	setupButtonUI(button_createThread, "Dialog", 18, 150, Pos.CENTER, 130, 540); // 130
     	button_createThread.setOnAction((_) -> showCreateThreadDialog());
     	
     	setupButtonUI(button_editThread, "Dialog", 18, 150, Pos.CENTER, 290, 540); // 290
     	button_editThread.setOnAction((_) -> showEditThreadDialog());
     	
     	setupButtonUI(button_deleteThread, "Dialog", 18, 150, Pos.CENTER, 450, 540);
     	button_deleteThread.setOnAction((_) -> showDeleteThreadDialog());
		...
		// Place all of the widget items into the Root Pane's list of children
         theRootPane.getChildren().addAll(
	        ...
	        line_Separator4, text_searchBar, threadChoice, button_createThread, button_CreatePost,
	        scrollFeed, button_Logout, button_editThread, button_deleteThread, button_ViewFlags, button_Quit);
}
	...
	private void refreshPostFeed(VBox container, String keyword, String thread) {
	    ...
	    for (post p : results) {
	        ...
	        button_post.setOnAction((_) -> showFullPost(p));
		     ...
		     flagButton.setStyle("-fx-font-size: 16px;");
	
		     // Show popup when clicked
		     flagButton.setOnAction(e -> showFlagDialog(p));
	
		     postRow.getChildren().addAll(button_post, flagButton);
		     container.getChildren().add(postRow);
	    }
	}
	
	/**********
	 * <p> Method: void showFlagDialog </p>
	 * 
	 * <p> Description: Prompts the user for a category and description for an admin request</p>
	 */
	private void showFlagDialog(post p) {
	    Stage dialog = new Stage();
	    dialog.initModality(Modality.APPLICATION_MODAL);
	    dialog.setTitle("Flag Post");
	    ...
	    descriptionField.setPromptText("Enter description (max 200)");
	    
	    TextField suggestedActionField = new TextField();
	    suggestedActionField.setPromptText("Enter suggested action (max 200)");

	    // Limit description length
	    descriptionField.textProperty().addListener((_, _, newVal) -> {
	        if (newVal.length() > 200) {
	            descriptionField.setText(newVal.substring(0, 200));
	        }
	    });
	    
	    suggestedActionField.textProperty().addListener((_, _, newVal) -> {
	        if (newVal.length() > 200) {
	            suggestedActionField.setText(newVal.substring(0, 200));
	        }
	    });

	    Button submit = new Button("Submit Flag");

	    submit.setOnAction(e -> {
	        String category = categoryBox.getValue();
	        String description = descriptionField.getText().trim();
	        String suggestedAction = suggestedActionField.getText().trim();

	        if (description.isEmpty()) {
	            showValidationError("Description cannot be empty.");
	            return;
	        }
	        
	        if (suggestedAction.isEmpty()) {
	            showValidationError("Suggested action cannot be empty.");
	            return;
	        }

//	        // FIXME: connect to db
//	        System.out.println("FLAGGED POST ID: " + p.getPostID());
//	        System.out.println("Category: " + category);
//	        System.out.println("Suggested Action: " + suggestedAction);
//	        System.out.println("Description: " + description);
//	        System.out.println("Staff: " + theUser.getUserName());
//	        System.out.println("Author of Post: " + p.getAuthor());
	        
	        theDatabase.createFlag(
	        	    p.getPostID(),
	        	    theUser.getUserName(),
	        	    p.getAuthor(),
	        	    category,
	    ...
	    layout.getChildren().addAll(
	        new Label("Flag this post"),
	        categoryBox,
	        suggestedActionField,
	        descriptionField,
	        submit
	    );
	...
	/**********
	 * <p> Method: void showFlagDashboard() </p>
	 * 
	 * <p> Description: Shows a list of all flags </p>
	 */
	private void showFlagDashboard() {
	    Stage stage = new Stage();
	    stage.initModality(Modality.APPLICATION_MODAL);
	    stage.setTitle("Flagged Posts");

	    VBox layout = new VBox(10);
	    layout.setStyle("-fx-padding: 15;");

	    VBox flagList = new VBox(10);
	    ScrollPane scroll = new ScrollPane(flagList);
	    scroll.setFitToWidth(true);
	    scroll.setPrefHeight(400);


	    List<String> flags = theDatabase.getAllFlags();

	    for (String f : flags) {
	        Label label = new Label(f);
	        label.setWrapText(true);
	        label.setStyle("-fx-border-color: gray; -fx-padding: 8;");
	        flagList.getChildren().add(label);
	    }

	    layout.getChildren().addAll(new Label("All Flagged Posts"), scroll);
	...
	/**********
	 * <p> Method: void getThreadNames() </p>
	 * 
	 * <p> Description: This method gets all thread names from the database. The thread
	 * "My Posts" is </p>
	 */
	...
	/**********
	 * <p> Method: void showCreateThreadDialog() </p>
	 * 
	 * <p> Description: This method displays a new panel for a staff to create a thread. The
	 * panel gives a text field for the thread name and buttons to submit or cancel.</p>
	 */
	protected void showCreateThreadDialog() {
	    Stage dialog = new Stage();
	    dialog.initModality(Modality.APPLICATION_MODAL);
	    VBox layout = new VBox(10);
	    ...
	    TextField threadTitle = new TextField();
	    threadTitle.setPromptText("Thead (Max 25)"); // TODO: need to account for max thread name
	    
	    threadTitle.getStylesheets().add(getClass().getResource("/applicationMain/application.css").toExternalForm());
	    Label titleCount = new Label("0/32");
	    titleCount.setStyle("-fx-text-fill: gray;");

	    // keeps the input of content <= 32
	    threadTitle.textProperty().addListener((_, _, newValue) -> {
	        if (newValue.length() > 25) {threadTitle.setText(newValue.substring(0, 32));}
	        titleCount.setText(threadTitle.getText().length() + "/32");
	    });
	    ...
	    submit.setOnAction((_) -> {
	        ...
	        if (title.isEmpty()) {
	        	...
	        	theDatabase.createThread(threadTitle.getText());
	        	dialog.close();
	        	refreshThreadComboBox();
	        	System.out.println("THREAD " + threadTitle.getText() + " CREATED");
	        } 
	    });
	    submit.getStylesheets().add(getClass().getResource("/applicationMain/application.css").toExternalForm());
	    ...
	    layout.getChildren().addAll(new Label("New Discussion"), threadTitle, titleCount, submit, cancel);
	    dialog.setScene(new Scene(layout));
	    dialog.show();
	}
	
	/**********
	 * <p> Method: void refreshThreadComboBox() </p>
	 * 
	 * <p> Description: This method refreshes the drop down box to select threads on the main page.</p>
	 */
	protected void refreshThreadComboBox() {
		List<String> list = getThreadNames();
		threadChoice.setItems(FXCollections.observableArrayList(list));
		threadChoice.getSelectionModel().select(0);
	}
	...
	/**********
	 * <p> Method: void showEditThreadDialog() </p>
	 * 
	 * <p> Description: This method displays a new panel for a staff to edit a thread. The
	 * panel gives a text field for the thread name, a drop down box to choose a thread,
	 * and buttons to submit or cancel. If the user confirms, all posts and replies under
	 * that thread will be moved to "General"</p>
	 */
	protected void showEditThreadDialog() {
	    Stage dialog = new Stage();
	    dialog.initModality(Modality.APPLICATION_MODAL);
	    VBox layout = new VBox(10);
	    layout.setPrefWidth(250);
		...
		pickThread.setLayoutY(55);
		List<String> dlist = getThreadNames();
		dlist.remove(0); // remove my posts thread
		dlist.remove(0); // remove general thread

		pickThread.setItems(FXCollections.observableArrayList(dlist));
		pickThread.getSelectionModel().select(0);
	    
	    TextField threadTitle = new TextField();
	    threadTitle.setPromptText("New Thread (Max 25)"); // TODO: need to account for max thread name
	    ...
	    submit.setOnAction((_) -> {
	        String newTitle = threadTitle.getText().trim();
	        ...
	        if (newTitle.isEmpty()) {
	        	...
	        	// Edit all threads under the original thread.
	        	theDatabase.editThread(pickThread.getValue(), newTitle);
	        	dialog.close();
	        	refreshThreadComboBox();
	        	System.out.println("THREAD " + pickThread.getValue() + " CHANGED TO " + newTitle);
	        }
	    ...
	    layout.getChildren().addAll(new Label("Edit Thread"), threadTitle, titleCount, pickThread, submit, cancel);
	    dialog.setScene(new Scene(layout));
	    dialog.show();
	}
	
	/**********
	 * <p> Method: void showDeleteThreadDialog() </p>
	 * 
	 * <p> Description: This method displays a new panel for a staff to delete a thread. The
	 * panel gives a drop down box to choose a thread and buttons to submit or cancel. If the
	 * user confirms, all posts and replies under that thread will be moved to "General"</p>
	 */
	protected void showDeleteThreadDialog() {
	    Stage dialog = new Stage();
	    dialog.initModality(Modality.APPLICATION_MODAL);
	    VBox layout = new VBox(10);
	    layout.setPrefWidth(250);
	    ...
	    submit.setOnAction((_) -> {
        	...
        	theDatabase.deleteThread(pickThread.getValue());
        	dialog.close();
        	refreshThreadComboBox();
        	System.out.println("THREAD " + pickThread.getValue() + " DELETED");
	    });
	    submit.getStylesheets().add(getClass().getResource("/applicationMain/application.css").toExternalForm());
	    ...
	    layout.getChildren().addAll(new Label("Delete Thread"), pickThread, submit, cancel);
	    dialog.setScene(new Scene(layout));
	    dialog.show();
	}
